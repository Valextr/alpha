...
class IBKRClient:
    ...
    def connect(self, timeout: int = 30) -> bool:
        ...
        try:
            ...
            # Pre-populate account summary cache
    ...
    def place_order(
        ...
        try:
            placed_trade = self._ib.placeOrder(contract, order)

            # ib_async returns a Trade object; get orderId from the underlying order
            placed_order = placed_trade.order if hasattr(placed_trade, 'order') else order
            logger.info(
                f"Order placed: orderId={placed_order.orderId}, "
                f"status={placed_trade.status if hasattr(placed_trade, 'status') else 'submitted'}"
            )
            return placed_trade
    ...
    def get_historical_data(
        ...
        bars = self._ib.reqHistoricalData(
            contract,
            endDateTime="",  # empty = use current time
        ...
        for fill in fills:
            result.append(
                FillRecord(
                    ...
                    action=fill.execution.side,  # execution.side = BUY/SELL
                    quantity=fill.execution.shares,
                    price=fill.execution.price,
                    date=str(fill.execution.time),
