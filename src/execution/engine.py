...
class ExecutionEngine:
    ...
    def __init__(
        ...
        monitoring_bridge: "MonitoringBridge | None" = None,
    ):
        self.config = config
        ...
        self.monitoring = monitoring_bridge
    ...
    def execute_targets(
        ...
        # Scale targets to fit within risk limits
        max_weight = self.risk_config.max_position_weight
        total_weight = sum(abs(w) for w in targets.values())
        
        if total_weight > 1.0:
            # Normalize if weights sum to more than 100%
            targets = {t: w / total_weight for t, w in targets.items()}
        
        # Check if any target exceeds max_position_weight
        exceeds_limit = any(abs(w) > max_weight for w in targets.values())
        if exceeds_limit:
            # Scale all targets down proportionally
            max_target = max(abs(w) for w in targets.values())
            scale_factor = max_weight / max_target
            targets = {t: w * scale_factor for t, w in targets.items()}
            logger.info(f"Scaled targets to fit within {max_weight:.0%} position limit: {targets}")
    ...
    def update_monitoring(self) -> None:
        """Push current state to the monitoring bridge.

        Call this after execute_targets() to update metrics and
    ...
    def record_end_of_day(self) -> float:
        ...
        if isinstance(self.broker, PaperBroker):
            daily_pnl = self.broker.record_eod()
        else:
            # For live brokers, calculate manually
            current_value = self.get_portfolio_value()
            daily_pnl = current_value - self._peak_equity
            self._daily_pnl = daily_pnl

        # Update monitoring if attached
        if self.monitoring:
            self.monitoring.record_end_of_day()
