...
class RiskGuard:
    ...
    def _check_cash_reserve(self, order: Order) -> tuple[bool, str]:
        ...
        if order.side == Side.BUY and order.avg_fill_price:
            ...
            # After buying, portfolio value stays the same (cash → stocks)
            # We need to ensure we keep min_cash_reserve as actual cash
            required_reserve = self.portfolio_value * self.config.min_cash_reserve
            remaining_cash = self.cash - order_cost
            if remaining_cash < required_reserve:
                # Scale down the order to fit within cash reserve
                max_order_cost = self.cash - required_reserve
                if max_order_cost <= 0:
                    return False, f"CASH_RESERVE: No cash available after ${required_reserve:,.0f} reserve"
                max_qty = int(max_order_cost / order.avg_fill_price)
                if max_qty < order.quantity:
                    # Order is too large — reject and let the engine scale it
                    return False, f"CASH_RESERVE: Max order ${max_order_cost:,.0f} (need ${required_reserve:,.0f} reserve)"
        return True, "APPROVED"
